...
|---|---|---|---|
| Личка-only | Только через DM с ботом | В группе не участвует | `TELEGRAM_ALLOWED_USERS`, `privacy mode = Enable`, `telegram.require_mention = true` |
| Группа без реакции на обычные сообщения | DM + `@botname` в рабочей группе | Молчит на обычные сообщения людей; отвечает только на `@mention` и ответы на свои сообщения | То же самое, плюс не делать бота админом |
...
| Владелец-only | Только DM владельца | В группе не участвует вообще | В `TELEGRAM_ALLOWED_USERS` оставить только один numeric user ID, не добавлять бота в группы, `telegram.require_mention = true` |
...
### Владелец пишет, группа читается пассивно

Отдельный желаемый сценарий, который отличается от `mention-only`:

- в личный чат может писать только владелец;
- бот может состоять в рабочей группе и читать её как контекст;
- бот не отвечает на `@mention` в группе;
- бот не отвечает на сообщения других людей в группе;
- исходящие сообщения в группу допустимы только как delivery-канал по задаче владельца.

В терминах конфигурации это не равняется `telegram.require_mention = true`.
Для такого режима нужен отдельный inbound-фильтр Telegram, который отдельно
разрешает owner DM и отдельно запрещает group replies. В текущем репозитории
этот режим пока зафиксирован как сценарий, а не как готовая настройка.

По умолчанию в этом репозитории включён именно режим без реакции на обычные
сообщения для групп. Это безопаснее для рабочих групп и даёт понятное
разделение:

- личка — для команд и контроля;
- `@mention` — для адресных запросов в рабочей группе;
- `home channel` — для отправки исходящих апдейтов;
- группа без реакции на обычные сообщения — когда бот присутствует в чате, но
  не отвечает на сообщения других людей, пока его явно не вызвали;
- `owner-only` — если никто, кроме владельца, не должен иметь вообще никакого канала общения;
...
рабочих групп безопаснее держать режим без реакции на обычные сообщения как
базовый.

Про `trusted IP`: это не Telegram-ограничение. В этой схеме Telegram-доступ
контролируется только `TELEGRAM_ALLOWED_USERS` и настройками самого bot flow
(`privacy mode`, `require_mention`, `home channel`). Если где-то ещё у тебя
есть вход по доверенным IP, это отдельный канал доступа, не замена Telegram
allowlist.

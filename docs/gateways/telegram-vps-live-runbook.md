...
> **Принцип (зафиксировано по итогам отладки): на этом VPS для STT нужен хостовый
> (облачный) провайдер, а не локальная модель.** VPS — CPU-only, 3 GB RAM. В такой
> объём влезает только whisper `small` (~2 GB), а он для живого русского даёт
> «средне»; `medium`/`large`, которые дали бы хорошее качество, не помещаются.
> Поэтому распознавание уводится в облачный аудио-провайдер (OpenRouter), который
> не грузит CPU/RAM VPS, лучше по русскому и стоит копейки за голосовое. Локальный
> whisper на этом контуре использовать не рекомендуется. Проверено на проде:
> расшифровка стала точной, ответы — по делу.
...
Важная мелочь, на которой залипли: ключ OpenRouter (`sk-or-v1-…`) на этом VPS был
записан в `.env` под именем `OPENAI_API_KEY` (Hermes использует его через
OpenAI-совместимый клиент). Поэтому агент отвечал текстом, но переменной
`OPENROUTER_API_KEY`, которую читает STT-обёртка, не было. Решение — продублировать
тот же ключ строкой `OPENROUTER_API_KEY=` в `config/.env` и пересоздать контейнер.

...
`google_calendar_os` также включён по умолчанию. Skill
`google-calendar-os` связывает Google Calendar с `todoist-os`: учитывает
фиксированные события, оценки и context labels, объединяет физические дела по
маршруту и использует безопасные окна в общественном транспорте или ожидании
для коротких телефонных задач.

...
Для разбора Inbox сначала найди реальный проект Inbox через `find-projects`,
затем вызови `find-tasks` с его ID. Обзор, labels и sections для этого не
обязательны: ошибка вспомогательного вызова не должна останавливать чтение или
обработку задач. Не передавай необязательные параметры как `null`, пустую
строку или придуманное значение. В частности, `cursor` можно передавать только
если предыдущий ответ вернул непустой cursor.

Если Todoist вернул `INVALID_ARGUMENT_VALUE`, используй поле `argument` из
ошибки: исправь или исключи именно этот параметр. Не повторяй тот же вызов с
теми же аргументами. Такая ошибка не означает, что Todoist недоступен. О сбое
сервиса можно сообщать только после cooldown и повторного вызова с
исправленными аргументами, если он завершился сетевой ошибкой, timeout или
server error.
...
Скрипт заменяет предыдущую managed-версию правил, а не добавляет ещё одну
копию.
...
`setup-macos.sh` устанавливает skill `google-calendar-os` в
`~/.hermes/skills/google-calendar-os`. Он работает поверх встроенного
`google-workspace` и совместно с `todoist-os`.

Текущая модель сохраняется: работа, учёба и личные события находятся в одном
primary-календаре и различаются названиями и emoji. Это даёт единую шкалу
времени для проверки конфликтов.

При планировании skill:

- сначала сохраняет фиксированные события, поездки, сон и встречи;
- использует estimate/context/mode из Todoist;
- ставит глубокую работу в защищённые блоки;
- объединяет физические задачи с уже запланированным маршрутом;
- использует поездку пассажиром или ожидание для коротких задач с меткой
  `📱 Телефон`;
- никогда не предлагает интерактивную работу во время вождения;
- добавляет подготовку, дорогу, восстановление и свободный резерв;
- показывает план и требует подтверждение перед созданием или удалением
  событий.

Для физических событий адрес следует записывать в Calendar `location`, чтобы
маршрутная оптимизация не зависела от свободного текста description.
